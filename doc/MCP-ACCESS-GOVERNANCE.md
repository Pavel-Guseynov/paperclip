...
| Gateway calls | `GET /api/tool-gateway/tools`, `POST /api/tool-gateway/tools/call` | Run-scoped session token, sent either in the `X-Paperclip-Tool-Gateway-Token` header or as `Authorization: Bearer pcgt_…`; the header wins when both are present. A `pcgw_…` named-gateway token is rejected here with `reasonCode: session_invalid` — it belongs on the gateway endpoints. Call body uses `tool` + `parameters`. Approval-required calls respond `409` with `reasonCode: approval_required` and an `actionRequestId`; the agent retries with `approvedActionRequestId`. |
